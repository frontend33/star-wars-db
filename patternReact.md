...
**Контекст CONTEXT**
Контекст позволяет создать хранилище данных и данные которые мы запишем в хранилище будут доступны всем компонентам иерархии без необходимости передавать эти данные через свойства 
 ...
 <App>
    <Provider value={lang}>
        ...
        <SupportPage>
            ...
            <Consumer>
                {
// В эту функцию компонент Consumer сможет передать, то значение которое он получит от ближайшего компонента в нашем случае значение lang из Provider 
                    (lang) => {
                        return (
