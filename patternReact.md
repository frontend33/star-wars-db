...
### withRouter()
 ...
 В path параметры могут быть опциональными благодаря в конце урл ?
...
Адрес должен содержать ID открытого элемента (тогда открыв URL пользователь попадает на тот же экран)
 ...
 ### Redirect
 Можно использовать компонент Redirect что бы переслать пользователя на логин страницу
 Система не обеспечивает безопасность в приложении (проверка прав осуществляется на сервере)
 ...
 Switch отрисует только первый элемент который соответствует адресу
  ...
  ```
  Route без свойства path сработает всегда